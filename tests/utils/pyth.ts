...
export class PythUtils {
    ...
    constructor(conn: Connection, wallet: NodeWallet) {
        this.conn = conn;
        this.wallet = wallet;
    ...
    // TODO MOVE
    async updateMetadataAccount(account: Keypair, data: Metadata) {
        const buf = Buffer.alloc(679);
        writeMetadataBuffer(buf, 0, data);
        await this.config.store(account, 0, buf);
    }
}
...
function writeMetadataBuffer(buf: Buffer, offset: number, data: Metadata) {
    buf.writeUIntLE(1, offset, 1)
    writePublicKeyBuffer(buf, offset + 1, PublicKey.default);
    writePublicKeyBuffer(buf, offset + 32, PublicKey.default);
