...
describe("jet", async () => {
  ...
  function bn(z: number): anchor.BN {
  ...
  async function createTestUser(
    ...
    const userClient = new HoneyClient(userProgram);

    return {
      wallet: userWallet,
      tokenAccounts,
      client: await HoneyUser.load(userClient, market, userWallet.publicKey),
    };
  }
  ...
  it("creates lending market", async () => {
    HoneyMarket = await client.createMarket({
      owner: marketOwner.publicKey,
      quoteCurrencyMint: usdc.token.publicKey,
      quoteCurrencyName: "USD",
    } as CreateMarketParams);

    userA = await createTestUser([usdc, wsol], HoneyMarket);
    userB = await createTestUser([usdc, wsol], HoneyMarket);
  });

  it("creates reserves", async () => {
    for (let tokenEnv of [usdc, wsol]) {
      tokenEnv.reserve = await HoneyMarket.createReserve({
  ...
  it("halts deposits", async () => {
    await HoneyMarket.setFlags(new splToken.u64(MarketFlags.HaltDeposits));
    ...
    await HoneyMarket.setFlags(new splToken.u64(0));
  });
  ...
  it("halts borrows", async () => {
    await HoneyMarket.setFlags(new splToken.u64(MarketFlags.HaltBorrows));
  ...
  it("halts repays", async () => {
    await HoneyMarket.setFlags(new splToken.u64(MarketFlags.HaltRepays));
